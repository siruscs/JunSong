<template>
  <view class="page" v-if="authorized">
    ...
    <view class="summary-bar" v-if="summary">
      <view><text class="summary-value">{{ quantity(summary.purchaseQuantity) }}<text class="summary-gift">赠{{ quantity(summary.giftQuantity) }}</text></text><text class="summary-label">数量</text></view>
    ...
    <view class="overlay-mask" v-if="panel === 'detail'" @tap="closePanel">
      ...
      >
        ...
        <view class="detail-footer-bar" v-if="detail && detail.purchaseId">
          ...
          <button v-if="can('bind') && detail.customerType === 'WALK_IN' && !detail.memberId" class="detail-action-btn bind-btn" @tap="openBind(detail)">绑定</button>
...
export default {
  ...
  methods: {
    ...
    onDetailTouchEnd() { const s = this.detailSwipeStart; if (!s) return; const threshold = 100; const velocityThreshold = 300; const dt = Math.max(1, Date.now() - s.startedAt); const vx = (this.detailSwipeX * 1000) / dt; if (this.detailSwipeX > threshold || vx > velocityThreshold) { this.detailSwipeClosing = true; this.detailSwipeX = Math.max(typeof screen !== 'undefined' ? screen.width : 420, 420); setTimeout(() => { this.closePanel(); this.detailSwipeStart = null; this.detailSwipeX = 0; this.detailSwipeClosing = false }, 220) } else { this.detailSwipeClosing = true; this.detailSwipeX = 0; setTimeout(() => { this.detailSwipeStart = null; this.detailSwipeClosing = false }, 220) } },
