    ...
    <view v-if="editorVisible" class="mask" @tap="closeEditor">
      <view class="sheet" @tap.stop>
        ...
        <scroll-view scroll-y class="form-scroll">
          ...
          <view class="field"><text class="label">商品</text><picker :range="products" range-key="productName" :value="productIndex" @change="changeProduct"><view class="picker">{{ selectedProduct.productName || '请选择商品' }} <text>⌄</text></view></picker></view>
          <view class="grid">
            <view class="field"><text class="label">调整数量</text><input class="input" type="digit" v-model="form.quantity" placeholder="" /><text class="field-hint">最多三位小数</text></view>
            <view class="field"><text class="label">单位成本（元）</text><input class="input" type="digit" v-model="form.unitCost" placeholder="" /><text class="field-hint">最多两位小数</text></view>
          </view>
...
function createEmptyForm() {
  return { deptId: null, adjustmentDate: new Date().toISOString().slice(0, 10), adjustmentType: '', adjustmentDirection: 'INCREASE', quantity: '', unitCost: '', productId: null, remark: '', items: [] }
}

export default {
  data() { return { rows: [], loading: false, refreshing: false, currentDeptId: null, currentDeptName: '', editorVisible: false, detailVisible: false, submitting: false, editingId: null, detail: null, products: [], typeOptions: [], typeIndex: 0, productIndex: -1, form: createEmptyForm() } },
  computed: {
    selectedType() { return this.typeOptions[this.typeIndex] || {} },
    selectedProduct() { return this.products[this.productIndex] || {} },
  ...
  methods: {
    ...
    async openEdit(item) { await this.loadOptions(); this.editingId = item.batchId; this.form = { ...this.emptyForm(), ...item, deptId: this.currentDeptId, items: this.detail?.data?.items || this.detail?.items || [] }; this.typeIndex = Math.max(0, this.typeOptions.findIndex(x => x.value === this.form.adjustmentType)); const first = this.form.items[0] || {}; this.form.productId = first.productId; this.form.quantity = first.quantity; this.form.unitCost = first.unitCost; this.productIndex = Math.max(0, this.products.findIndex(x => String(x.productId) === String(first.productId))); this.editorVisible = true },
    ...
    changeProduct(e) { this.productIndex = Number(e.detail.value); const p = this.selectedProduct; this.form.productId = p.productId; this.form.unitCost = Number(p.purchasePrice ?? p.buyPrice ?? p.price ?? 0).toFixed(2) },
    ...
    async save() { const quantity = Number(this.form.quantity); const unitCost = Number(this.form.unitCost); if (!this.currentDeptId || !this.form.adjustmentType || !this.form.productId) return uni.showToast({ title: '请完整填写调整类型和商品', icon: 'none' }); if (!Number.isFinite(quantity) || quantity <= 0 || String(this.form.quantity).split('.')[1]?.length > 3) return uni.showToast({ title: '调整数量必须大于0且最多三位小数', icon: 'none' }); if (!Number.isFinite(unitCost) || unitCost < 0 || String(this.form.unitCost).split('.')[1]?.length > 2) return uni.showToast({ title: '单位成本最多两位小数', icon: 'none' }); this.submitting = true; const data = { deptId: this.currentDeptId, adjustmentDate: this.form.adjustmentDate, initDate: this.form.adjustmentDate, adjustmentType: this.form.adjustmentType, adjustmentDirection: this.form.adjustmentDirection, remark: this.form.remark, items: [{ productId: this.form.productId, quantity, unitCost }] }; try { if (this.editingId) await updateStockInit(this.editingId, { ...data, version: this.form.version }); else await createStockInit(data); uni.showToast({ title: '保存成功', icon: 'success' }); this.editorVisible = false; await this.load() } catch (e) { uni.showToast({ title: e.msg || '保存失败', icon: 'none' }) } finally { this.submitting = false } },
