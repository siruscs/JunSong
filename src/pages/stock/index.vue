<template>
  <view class="page">
    ...
    <scroll-view scroll-y class="scroll" refresher-enabled :refresher-triggered="refreshing" @refresherrefresh="refresh">
      <view class="stock-card" v-for="item in items" :key="`${item.deptId}-${item.productId}`">
  ...
  methods: {
    ...
    load() {
      ...
      getStockValueReport({ startDate: '', endDate: '', pageNum: 1, pageSize: 200 })
        .then(res => { const data = res.data || {}; this.report = data; this.items = data.items || [] })
