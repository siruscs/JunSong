...
public class FinExpenseVerificationServiceImpl implements IFinExpenseVerificationService
{
    ...
    public Long verify(ExpenseVerifyVO request, String operator)
    {
        ...
        if (!advances.isEmpty() && difference.signum()!=0) { boolean supplement=difference.signum()>0; FinAdvance generated=new FinAdvance(); generated.setDeptId(deptId); generated.setPeriodId(advances.get(0).getPeriodId()); generated.setAdvanceDate(now); generated.setAdvanceAmount(difference.abs()); generated.setAdvanceNo("TZ"+now.getTime()); generated.setBorrower(advances.get(0).getBorrower()); generated.setPurpose(supplement ? "费用核销补款" : "费用核销节余"); generated.setStatus(supplement ? "1" : "0"); generated.setVerifyBy(supplement ? operator : null); generated.setVerifyTime(supplement ? now : null); generated.setDelFlag("0"); generated.setCreateBy(operator); if(advanceMapper.insertFinAdvance(generated)!=1||generated.getAdvanceId()==null)throw new ServiceException("核销差额记录生成失败"); advanceDetails.add(detail(batch.getBatchId(), tenantId, deptId, generated, supplement ? FinAdvanceVerifyDetail.RELATION_SUPPLEMENT : FinAdvanceVerifyDetail.RELATION_SURPLUS, "1", now)); }
