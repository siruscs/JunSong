...
DELETE FROM sys_role_menu WHERE menu_id IN (
    SELECT menu_id FROM sys_menu WHERE menu_name = '会员服务' AND parent_id = 0
);

DELETE FROM sys_menu WHERE menu_name = '会员服务' AND parent_id = 0;
...
VALUES (@maxMenuId + 1, '会员服务', 0, 6, 'member', NULL, 1, 0, 'M', '0', '0', '', 'el-icon-user-solid', 'admin', NOW(), '', NULL, '会员服务目录');

-- 3. 创建会员信息子菜单（修改path和component为member/member）
INSERT INTO `sys_menu` (`menu_id`, `menu_name`, `parent_id`, `order_num`, `path`, `component`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `update_by`, `update_time`, `remark`)
VALUES 
(@maxMenuId + 2, '会员信息', @maxMenuId + 1, 1, 'member', 'member/member/index', 1, 0, 'C', '0', '0', 'member:member:list', 'el-icon-user', 'admin', NOW(), '', NULL, '会员信息菜单'),
...
SELECT 'Menus recreated successfully' AS status;
