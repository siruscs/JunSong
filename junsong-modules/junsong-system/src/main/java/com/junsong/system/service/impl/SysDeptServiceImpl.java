...
public class SysDeptServiceImpl implements ISysDeptService
{
    ...
    /** 门店地图缓存键 */
    private static final String STORE_GEO_CACHE_KEY = "store:geo:all";
    /** 顶级部门地图缓存键 */
    private static final String TOP_DEPT_GEO_CACHE_KEY = "dept:geo:top";
    /** 缓存过期时间（分钟） */
    private static final long GEO_CACHE_TTL_MINUTES = 10;
    ...
    public int insertDept(SysDept dept)
    {
        ...
        {
            throw new ServiceException("上级部门不存在，不允许新增");
        }
    ...
    {
        return deptMapper.selectStoreGeoInBbox(minLng, maxLng, minLat, maxLat);
    }
    ...
    {
        // 优先从 Redis 缓存读取
        List<SysDept> cached = redisService.getCacheObject(STORE_GEO_CACHE_KEY);
        if (cached != null)
        {
            return cached;
        }
        List<SysDept> result = deptMapper.selectStoreGeoAll();
        redisService.setCacheObject(STORE_GEO_CACHE_KEY, result, GEO_CACHE_TTL_MINUTES, TimeUnit.MINUTES);
        return result;
    }
    ...
    {
        // 优先从 Redis 缓存读取
        List<SysDept> cached = redisService.getCacheObject(TOP_DEPT_GEO_CACHE_KEY);
        if (cached != null)
        {
            return cached;
        }
        List<SysDept> result = deptMapper.selectTopDeptGeoAll();
        redisService.setCacheObject(TOP_DEPT_GEO_CACHE_KEY, result, GEO_CACHE_TTL_MINUTES, TimeUnit.MINUTES);
        return result;
    }
    ...
    {
        return deptMapper.selectStoreGeoByRegion(dept);
    }
}
