...
function operationLabel(operation: string) { return ({ CREATE: '目标机构没有此配置，将新增', DIFF: '目标机构已有配置，内容有差异', NOOP: '目标机构已有相同配置，无需处理', CONFLICT: '编码已被其他机构占用，无法新增', IMPACT_BLOCKED: '目标等级已被会员使用，禁止自动覆盖', PRODUCT_MISSING: '目标机构缺少商品，将同时同步' } as Record<string, string>)[operation] || '需要人工确认' }
function operationType(operation: string) { return operation === 'NOOP' ? 'success' : operation === 'CONFLICT' || operation === 'IMPACT_BLOCKED' ? 'danger' : operation === 'PRODUCT_MISSING' ? 'warning' : 'warning' }
