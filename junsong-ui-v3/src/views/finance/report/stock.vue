  ...
  <div class="app-container report-page">
    ...
    <div class="report-metrics">
      <div class="metric-card primary">
        ...
        <div class="metric-value">{{ reportData.totalStock || 0 }}</div>
      </div>
      <div class="metric-card success">
        <div class="metric-label">库存金额</div>
        <div class="metric-value">¥{{ reportData.totalValue || 0 }}</div>
      </div>
      <div class="metric-card warning">
        <div class="metric-label">低库存商品</div>
        <div class="metric-value">{{ reportData.lowStockCount || 0 }}</div>
      </div>
    </div>

    <div class="chart-grid">
      <el-card class="chart-card">
        <template #header><span>门店库存对比</span></template>
...
export default {
  ...
  methods: {
    ...
    reRenderCharts() {
      ...
      this.initDeptChart && this.initDeptChart();
      this.initCategoryChart && this.initCategoryChart();
    },
    ...
    getLineOptions(labels, values) {
      const data = values || [];
      return {
        color: [this.getThemeColor()],
        tooltip: { trigger: "axis" },
        grid: { left: 34, right: 22, top: 32, bottom: 36, containLabel: true },
        graphic: this.getEmptyGraphic(data),
        xAxis: {
          ...this.getAxisBase(),
          type: "category",
        ...
        series: [{
          ...
          symbolSize: 7,
          data,
          lineStyle: { width: 3, color: this.getThemeColor() },
          itemStyle: { color: this.getThemeColor(), borderWidth: 2, borderColor: "#fff" },
          areaStyle: { color: this.getThemeRgba(0.12) }
        }]
      };
    },
    getPieOptions(data) {
    ...
    initDeptChart() {
      ...
      this.deptChart.setOption(this.getBarOptions(data.map(item => item.deptName), data.map(item => item.stockValue)));
    },
    initCategoryChart() {
      ...
      this.categoryChart.setOption(this.getPieOptions(data.map(item => ({ name: item.categoryName, value: item.stockCount }))));
    }
  }
};
</script>
