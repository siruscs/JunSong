...
print(f"Deleted {cursor.rowcount} old row(s)")
cursor.execute("INSERT INTO config_info (data_id, group_id, content, md5, gmt_create, gmt_modified, src_user, src_ip, app_name, tenant_id, c_desc, c_use, effect, type, c_schema) VALUES ('junsong-job-dev.yml', 'DEFAULT_GROUP', %s, '', NOW(), NOW(), 'admin', '127.0.0.1', '', '', '', '', 'Y', 'yaml', '')", (config,))
conn.commit()
print(f"Inserted new config, len={len(config)}")
cursor.execute("SELECT id, LENGTH(content) FROM config_info WHERE data_id = 'junsong-job-dev.yml'")
row = cursor.fetchone()
print(f"Verify: id={row[0]}, len={row[1]}")
cursor.close()
conn.close()
