<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="policies" border>
      ...
      <el-table-column label="操作" fixed="right" width="260"><template #default="scope"><el-button link type="primary" @click="openEdit(scope.row)">查看/编辑</el-button><el-button link type="warning" @click="toggleStatus(scope.row)">{{ String(scope.row.status) === '1' ? '停用' : '启用' }}</el-button><el-button link type="success" @click="openSync(scope.row)" v-hasPermi="['member:campaignPolicy:sync']">同步到其他机构</el-button></template></el-table-column>
    </el-table>
    <el-dialog v-model="dialogOpen" :title="editing ? '查看/编辑销售政策' : '新增销售政策'" width="820px">
      <el-form :model="form" label-width="110px">
        <el-row :gutter="16"><el-col :span="12"><el-form-item label="政策编号"><el-input v-model="form.policyNo" disabled /></el-form-item></el-col><el-col :span="12"><el-form-item label="政策名称"><el-input v-model="form.policyName" placeholder="自动生成，可修改" /></el-form-item></el-col></el-row>
        <el-row :gutter="16"><el-col :span="12"><el-form-item label="核算周期"><el-select v-model="form.periodId" placeholder="请选择核算周期" filterable style="width: 100%" @change="generatePolicyNo"><el-option v-for="period in periods" :key="period.periodId" :label="periodLabel(period)" :value="period.periodId" /></el-select></el-form-item></el-col><el-col :span="12"><el-form-item label="商品"><el-select v-model="form.productId" filterable placeholder="请选择商品" style="width: 100%" @change="generatePolicyNo"><el-option v-for="product in products" :key="product.productId" :label="product.productName" :value="product.productId" /></el-select></el-form-item></el-col></el-row>
...
import { addCampaignPolicy, changeCampaignPolicyStatus, listCampaignPolicies, updateCampaignPolicy } from '@/api/member/campaignPolicy'
...
function generatePolicyNo() { if (editing.value || !form.periodId || !form.productId) return; const product = products.value.find(item => item.productId === form.productId); const period = periods.value.find(item => item.periodId === form.periodId); const code = String(product?.productCode || form.productId).replace(/[^A-Za-z0-9]/g, '').toUpperCase(); form.policyNo = `${period?.periodNo || form.periodId}-${code}-${Date.now().toString().slice(-4)}`; if (!form.policyName) form.policyName = `${product?.productName || '商品'}销售政策` }
function openCreate() { editing.value = false; Object.assign(form, { policyId: undefined, policyNo: '', policyName: '', periodId: query.periodId || periods.value.find(item => String(item.status) === '1')?.periodId, productId: query.productId, version: 1, customerScope: 'MEMBER', effectiveStart: undefined, effectiveEnd: undefined, status: '1', packages: [blankPackage()] }); generatePolicyNo(); syncEffectiveDates(); dialogOpen.value = true }
